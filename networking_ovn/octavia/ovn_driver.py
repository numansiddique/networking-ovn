...
external_ids  : {
    ...
    vip="10.0.0.10", vip_network_id="2526c68a-5a9e-484c-8e00-0716388f6563"}
...
external_ids  : {
    lr_ref="neutron-52b6299c-6e38-4226-a275-77370296f257",
    ls_refs="{\"neutron-2526c68a-5a9e-484c-8e00-0716388f6563\": 1}",
    "pool_f2ddf7a6-4047-4cc9-97be-1d1a6c47ece9"="", vip="10.0.0.10",
    vip_network_id="2526c68a-5a9e-484c-8e00-0716388f6563"}
name          : "973a201a-8787-4f6e-9b8f-ab9f93c31f44"
protocol      : []
vips          : {}
...
external_ids  : {
    ...
    ls_refs="{\"neutron-2526c68a-5a9e-484c-8e00-0716388f6563\": 2,
              \"neutron-12c42705-3e15-4e2d-8fc0-070d1b80b9ef\": 1}",
    "pool_f2ddf7a6-4047-4cc9-97be-1d1a6c47ece9"="10.0.0.107:80,20.0.0.107:80",
    "listener_82"="pool_f2ddf7a6-4047-4cc9-97be-1d1a6c47ece9",
    vip="10.0.0.10", vip_network_id="2526c68a-5a9e-484c-8e00-0716388f6563"}
name          : "973a201a-8787-4f6e-9b8f-ab9f93c31f44"
protocol      : []
...
class OvnProviderHelper(object):
    ...
    def _update_lb_to_ls_association(self, ovn_lb, network_id=None,
        ...
        ls_refs = ovn_lb.external_ids.get('ls_refs')
        if ls_refs:
            try:
        ...
        ls_refs = {'ls_refs': jsonutils.dumps(ls_refs)}
    ...
    def _refresh_lb_data(self, ovn_lb, updated_external_ids):
        ext_ids = {}
        if 'enabled' in updated_external_ids:
            ext_ids['enabled'] = updated_external_ids['enabled']
    ...
    def _get_pool_listeners(self, ovn_lb, pool_key):
        ...
        for k, v in ovn_lb.external_ids.items():
            if 'listener_' not in k:
                continue
            vip_port, p_key = self._extract_listener_key_value(v)
            if pool_key == p_key:
                pool_listeners.append(k[len('listener_'):])
    ...
    def _frame_vip_ips(self, lb_external_ids):
        ...
        lb_vip = lb_external_ids['vip']
        for k, v in lb_external_ids.items():
            if 'listener_' not in k or self._is_listener_disabled(k):
                continue
    ...
    def lb_create(self, loadbalancer):
        try:
            ...
            self.ovn_nbdb_api.db_create(
                ...
                external_ids={'vip': loadbalancer['vip_address'],
    ...
    def lb_delete(self, loadbalancer):
        try:
            ...
            lr_ref = ovn_lb.external_ids.get('lr_ref')
            if lr_ref:
            ...
            network_driver.neutron_client.delete_port(
                ovn_lb.external_ids['vip_port_id'])
        except Exception:
    ...
    def listener_update(self, listener):
        try:
            ...
            if 'default_pool_id' in listener:
                ...
                pool_status = {'id': listener['default_pool_id'],
                               'provisioning_status': constants.ACTIVE}
...
class OvnProviderDriver(driver_base.ProviderDriver):
    ...
    def listener_update(self, old_listener, new_listener):
        self._check_for_supported_protocols(new_listener.protocol)
