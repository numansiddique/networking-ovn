...
class TestOctaviaOvnProviderDriver(base.TestOVNFunctionalBase):
    ...
    def _get_loadbalancers(self):
        ...
        for lb in self.nb_api.tables['Load_Balancer'].rows.values():
            ...
            ls_refs = external_ids.get('ls_refs')
            if ls_refs:
                external_ids['ls_refs'] = jsonutils.loads(ls_refs)
    ...
    def _update_ls_refs(self, lb_data, net_id, add_ref=True):
        ...
        if add_ref:
            if net_id in lb_data['ls_refs']:
                ref_ct = lb_data['ls_refs'][net_id] + 1
                lb_data['ls_refs'][net_id] = ref_ct
            else:
                lb_data['ls_refs'][net_id] = 1
        else:
            ref_ct = lb_data['ls_refs'][net_id] - 1
            if ref_ct > 0:
                lb_data['ls_refs'][net_id] = ref_ct
            else:
                del lb_data['ls_refs'][net_id]
    ...
    def _create_load_balancer_and_validate(self, lb_info, admin_state_up=True):
        self._o_driver_lib.update_loadbalancer_status.reset_mock()
        lb_data = {}
        router_id = self._create_router("r1")
        lb_data['lr_ref'] = router_id
    ...
    def _update_load_balancer_and_validate(self, lb_data,
        ...
        self.ovn_driver.loadbalancer_update(lb_data['model'], lb_data['model'])
    ...
    def _make_expected_lbs(self, lb_data):
        ...
        external_ids = {'ls_refs': {},
                        'vip_network_id': vip_net_info[0],
                        'vip': lb_data['model'].vip_address,
                        'vip_port_id': vip_net_info[3],
        ...
        for net_id, ref_ct in lb_data['ls_refs'].items():
            external_ids['ls_refs'][net_id] = ref_ct

        if lb_data.get('lr_ref'):
            external_ids['lr_ref'] = 'neutron-' + lb_data['lr_ref']
    ...
    def _test_for_unsupported_options(self):
        lb_data = self._create_load_balancer_and_validate(
            {'vip_network': 'vip_network',
             'cidr': '10.0.0.0/24'})
        ...
        # Creating listener without default pool id should raise exception.
        m_listener.default_pool_id = None
        self.assertRaises(o_exceptions.UnsupportedOptionError,
                          self.ovn_driver.listener_create, m_listener)

        # load balancer failover should raise NotImplementedError exception.
        self.assertRaises(o_exceptions.NotImplementedError,
